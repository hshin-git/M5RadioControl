...
- M5RadioControlは、RCシステムの送受信機TX/RXをM5Stackマイコンで自作するためのオープンソースソフトウェアです。
- RCシステムの標準機能（ペアリング、トリム、リバース、D/R、EPA、EXP設定など）に加えて、独自機能を実装しています。
- 独自機能は、送信機TXの傾斜により操縦するダンシング機能、受信機RXの慣性計測値のテレメータ表示／ロギング機能等です。
...
M5RCシステムは、IMUテレメータ／ロギング機能により「動く実験室」を実現できます。
...
- 送信機TXは、"M5Stack Gray"とA/Dコンバータ(I2C接続)
- 受信機RXは、"M5Atom Matrix"または"M5StickC"
- ホビーRCのサーボ、ESC（電子式スピードコントローラ）
